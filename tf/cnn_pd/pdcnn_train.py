...
f1 = open(FLAGS.data_dir1 + '/Train.bin', 'rb')
data_train1 = np.fromstring(f1.read(), dtype="uint8")
data_train1 = data_train1.reshape(-1,resizedImgRows*resizedImgCols+1)
print("data_train1 size:{0}".format(data_train1.shape))
f1.close()
...
#data_validate = np.concatenate([data_validate1])
...
def nextBatch(batchSize):
	#print("data rows: {0}".format(data.shape[0]))
	global BatchNumber
	global data_train
	#print("BatchNumber: {0}".format(BatchNumber))
	startRow = batchSize*BatchNumber
	#print("startRow: {0}".format(startRow))
...
#x = tf.placeholder("float", shape=[None, resizedImgRows*resizedImgCols], name="input")
x = tf.placeholder("float32", shape=[None, resizedImgRows*resizedImgCols], name="input")
...
b_conv3 = bias_variable([Layer3_Filters], 'b_conv')
...
for i in range(FLAGS.max_steps):
	...
	if(validate_accuracy > 0.97):
    		...
    		tf.train.write_graph(sess.graph.as_graph_def(), FLAGS.train_dir, "graph.pb")
